...
class ImageConverter:
    def __init__(self, app):
        ...
        self.convert_gif_button = Button(app, text="Convert to GIF", command=self.convert_image_to_gif)
        ...
        self.contour_button = Button(app, text="Apply BLUR Filter", command=self.blur_filter)
        self.contour_button.grid(row=8, column=0)
    ...
    def open_image(self):
        ...
        self.image_path = filedialog.askopenfilename(filetypes=[("GIF Files", "*.gif"), ("JPEG Files", "*.jpg;*.jpeg")])
    ...
    def convert_image_to_gif(self):  # функція конвертації у GIF
        if self.original_image:
            jpeg_path = self.image_path[:-4] + ".gif"
            # зберігаємо зображення в форматі GIF
            self.original_image.save(jpeg_path, "GIF")
            self.convert_label.config(text="Saved in GIF")
            self.change_image(self.image_path)

    def convert_image_to_jpeg(self):  # функція конвертації у JPEG
        if self.original_image:
            gif_path = self.image_path[:-4] + ".jpg"
            # зберігаємо зображення в форматі JPEG
            image_rgb = self.original_image.convert('RGB')
            image_rgb.save(gif_path, "JPEG")
    ...
    def resize_image(self):  # функція для зміни розміру зображення
        if self.original_image and self.scale_x.get() and self.scale_y.get():
            ...
            self.original_image = self.original_image.resize((x_scale, y_scale))
    ...
    def change_scale(self):  # функція для зміни масштабу у к раз
        if self.original_image and self.scaling.get():
            ...
            width1 = k * width
            height1 = k * height
            self.original_image = self.original_image.resize((int(width1), int(height1)))
            self.original_image.save(self.image_path)
            self.scaling_label.config(text=f"Scale changed in {self.scaling.get()} times")
            self.change_image(self.image_path)

    def blur_filter(self):  # функція для фільтру BLUR
        if self.original_image:
            image_rgb = self.original_image.convert('RGB').filter(ImageFilter.BLUR)
            image_rgb.save(self.image_path)
